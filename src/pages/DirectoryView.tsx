...
const DirectoryView: React.FC<{ albums?: Album[] }> = () => { // Optional prop for initial data if needed later
  ...
  if (isLoading) {
    ...
    return <div className="text-center p-10 text-(--color-element-light) tracking-widest">Loading music library...</div>;
  }
  ...
  if (albums.length === 0) {
      return <div className="text-center p-10 text-(--color-element-light) tracking-widest">No albums found.</div>;
  }
    ...
    <div className="p-4">
      ...
      <div className="m-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4">
        {albums.map((album) => (
          ...
          >
            {/* Album Art */}
            <div className="aspect-square bg-(--color-content-light) flex items-center justify-center">
              {album.albumArtSrc ? (
                <img src={album.albumArtSrc} alt={`Cover for ${album.title}`} className="w-full h-full object-cover"/>
              ) : (
                 <span className="text-9xl text-(--color-element-light)">🎵</span> // Placeholder
              )}
            </div>
            {/* Album Info */}
            <div className="p-3 text-(--color-element-light)">
               <h3 className="font-semibold truncate font-title text-(--color-content-light) group-hover:text-mizu-dark dark:group-hover:text-mizu-light" title={album.title}>
                  {album.title}
                </h3>
