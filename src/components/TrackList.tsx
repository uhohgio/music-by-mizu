    ...
    <ul className="space-y-2"> {/* Adds vertical space between list items */}
      {tracks.map((track) => {
        ...
        return (
            ...
            // Style list items
            >
                <span className={` ${isActive ? 'font-bold text-[var(--color-content-light)] dark:text-[var(--color-content-dark)] pl-2' : 'font-semibold text-[var(--color-content-light)]'} pl-2`}> {/* Adjust text color based on active state */}
                {track.title} 
                </span>
